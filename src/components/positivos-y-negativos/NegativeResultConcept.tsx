...
export function NegativeResultConcept() {
        ...
        <div className="space-y-8">
            <div className="bg-white dark:bg-zinc-900 p-8 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold text-slate-800 dark:text-zinc-100 mb-4">
                    Cuando dominan las fichas negativas
                </h2>
                <p className="text-lg text-slate-600 dark:text-zinc-400 max-w-2xl">
                    Si tenemos más fichas negativas que positivas, después de cancelar todos los pares cero todavía nos sobrarán fichas rojas. Así, el resultado final será <strong>negativo</strong>.
                </p>
                ...
                <div className="mt-8 bg-slate-50 dark:bg-zinc-800/50 p-6 rounded-2xl flex flex-col md:flex-row items-center gap-8">
                        ...
                        <svg viewBox="0 0 300 200" className="w-full max-w-[250px] drop-shadow-md">
                            ...
                            <g>
                                ...
                                <g className={`transition-all duration-1000 ${animated ? 'delay-1000 opacity-0' : 'opacity-100'}`}>
                                    <rect x="50" y="65" width="30" height="30" rx="8" fill="#e0e7ff" stroke="#818cf8" strokeWidth="2" />
                                    <path d="M65 73 v14 M58 80 h14" stroke="#4f46e5" strokeWidth="3" strokeLinecap="round" />
                                </g>
                                {/* Tile 2 (Cancelled) */}
                                <g className={`transition-all duration-1000 ${animated ? 'delay-1000 opacity-0' : 'opacity-100'}`}>
                                    <rect x="50" y="110" width="30" height="30" rx="8" fill="#e0e7ff" stroke="#818cf8" strokeWidth="2" />
                                    <path d="M65 118 v14 M58 125 h14" stroke="#4f46e5" strokeWidth="3" strokeLinecap="round" />
                                </g>
                            </g>

                            {/* Negative Tiles */}
                            <g>
                                {/* Neg Tile 1 (Moves and cancels) */}
                                ...
                                {/* Neg Tile 2 (Moves and cancels) */}
                                <g className={`transition-all duration-1000 ease-in-out ${animated ? 'translate-x-[0px] translate-y-[35px] opacity-100' : 'opacity-100'} ${animated ? 'delay-1000 translate-x-[-80px] opacity-0': ''}`}>
                                ...
                                {/* Neg Tile 3 (Remains) */}
                                <g className={`transition-all duration-1000 ease-in-out ${animated ? 'translate-x-[0px] translate-y-[0px] delay-1000' : ''}`}>
                                    <rect x="130" y="85" width="30" height="30" rx="8" fill="#fee2e2" stroke="#f87171" strokeWidth="2" />
                                    <path d="M138 100 h14" stroke="#e11d48" strokeWidth="3" strokeLinecap="round" />
                                </g>
                                {/* Neg Tile 4 (Remains) */}
                                <g className={`transition-all duration-1000 ease-in-out ${animated ? 'delay-1000' : ''}`}>
                                    <rect x="130" y="130" width="30" height="30" rx="8" fill="#fee2e2" stroke="#f87171" strokeWidth="2" />
                                    <path d="M138 145 h14" stroke="#e11d48" strokeWidth="3" strokeLinecap="round" />
                                </g>
                            </g>

                            {/* Result bracket & text */}
                            <g className={`transition-all duration-1000 delay-[2000ms] ${animated ? 'opacity-100' : 'opacity-0'}`}>
                                <path d="M190 70 h15 v90 h-15" fill="none" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                <text x="220" y="125" fontSize="24" fontWeight="bold" fill="#334155" className="dark:fill-slate-200">-2</text>
                            </g>
                        </svg>
                    </div>
                    ...
                    <div className="w-full md:w-1/2 space-y-4">
                        <div className="bg-rose-50 dark:bg-rose-900/30 p-4 rounded-xl text-rose-900 dark:text-rose-200">
                            <strong>Explicación:</strong> Tenemos 2 fichas positivas y 4 negativas. Podemos cancelar hasta 2 pares cero. Una vez que se cancelan, sobran 2 fichas rojas. El resultado es <strong>-2</strong>.
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col items-center">
                        ...
                        {[-5, -3, 3, 5].map(num => (
                            <button
                                ...
                                className={`w-12 h-12 rounded-xl text-lg font-bold transition-all ${exercise1Answer === num 
                                    ? num === -3 ? 'bg-emerald-500 text-white shadow-lg' : 'bg-red-500 text-white' 
                    ...
                    {exercise1Answer === -3 && (
                        <div className="text-emerald-600 dark:text-emerald-400 text-sm font-semibold mt-4 animate-in fade-in slide-in-from-bottom-2 text-center">
                            ¡Bien! La ficha positiva cancela a una negativa, quedando 3 rojas (-3).
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col items-center">
                    <h3 className="font-semibold text-slate-700 dark:text-zinc-200 mb-4 self-start">
                        2. Coloca al menos 2 fichas POSITIVAS y logra que el total siga siendo -1.
                    </h3>
                    <div className="flex gap-4 items-center flex-col w-full">
                        ...
                        {(exercise2PosTiles >= 2) && (exercise2PosTiles - exercise2NegTiles === -1) && (
                            <div className="text-emerald-600 dark:text-emerald-400 text-sm font-semibold mt-4 animate-in fade-in slide-in-from-bottom-2 text-center">
                                ¡Excelente! Usaste {exercise2PosTiles} fichas positivas, pero al poner {exercise2NegTiles} negativas hiciste que ganen los negativos por 1.
                            </div>
                        )}
                    </div>
                </div>
