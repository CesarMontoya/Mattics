...
export function ProductSameBaseVisual() {
    const [activeExample, setActiveExample] = useState<1 | 2>(1)
    const [step, setStep] = useState(0)

    // Example 1: 2³ × 2² = 2⁵
    const ex1 = { base: 2, exp1: 3, exp2: 2, result: 5, finalValue: 32 }
    // Example 2: 3² × 3¹ = 3³
    const ex2 = { base: 3, exp1: 2, exp2: 1, result: 3, finalValue: 27 }
        ...
        <div className="flex flex-col gap-8 p-6 md:p-8 bg-white dark:bg-zinc-900 rounded-3xl border border-slate-100 dark:border-zinc-800 shadow-xl overflow-visible">
                        ...
                        {step === 2 && (
                            <g className="animate-in fade-in duration-700">
                                <text
                                    ...
                                    style={{ fontFamily: "'Quicksand', sans-serif" }}
                                >
                                    {Array(ex.exp1 + ex.exp2).fill(ex.base).join(' × ')}
                                </text>
                                <text
                                    x={300}
                                    y={160}
                                ...
                                >
                                    = {ex.base}
                                    <tspan className="text-xl" dy="-12" dx="3">{ex.exp1} + {ex.exp2}</tspan>
                                    <tspan className="text-2xl" dy="12" dx="5"> = </tspan>
                                    <tspan className="text-2xl" dy="0" dx="5">{ex.base}</tspan>
                                    <tspan className="text-xl" dy="-12" dx="3">{ex.result}</tspan>
                                </text>
                            </g>
                        )}
            ...
            <div className="flex flex-col items-center gap-4">
                <p className="text-lg text-slate-600 dark:text-zinc-400 font-medium text-center">
                    {step === 0 && `Al multiplicar potencias con la misma base, sumamos los exponentes`}
                    {step === 1 && `Expandimos cada potencia para ver todos los factores`}
                    {step === 2 && `Contamos los factores: ${ex.exp1} + ${ex.exp2} = ${ex.result} factores de ${ex.base}`}
